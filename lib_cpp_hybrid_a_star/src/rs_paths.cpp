...
namespace rs_paths
{
    ...
    void RSPaths::set_path(std::vector<Path>& paths, std::vector<double> lengths, std::vector<std::string> ctypes){
        ...
        path.L = std::accumulate(path.lengths.begin(), path.lengths.end(), 0.0);
    ...
    void RSPaths::generate_path(Eigen::Vector4d q0, Eigen::Vector4d q1, double maxc, std::vector<Path>& paths){
        double dx = q1[1] - q0[1];
        double dy = q1[2] - q0[2];
        double dth = q1[3] - q0[3];
        double c = cos(q0[3]);
        double s = sin(q0[3]);
    ...
    void RSPaths::generate_local_course(double L, std::vector<double> lengths
                ...
                int npoint = trunc(L/step_size) + static_cast<int>(lengths.size()) + 3;
    ...
    void RSPaths::calc_paths(Eigen::Vector4d s, Eigen::Vector4d g, double maxc,  std::vector<Path>& paths, double step_size){
        ...
        std::for_each(paths.begin(), paths.end(), [&](Path &path) {
            ...
            for(int i=0; i<poses.rows(); i++){
                ...
                double directions = poses.row(i)[3];
                poses.row(i) << cos(-q0[3]) * ix + sin(-q0[3]) * iy + q0[1], -sin(-q0[3]) * ix + cos(-q0[3]) * iy + q0[2]
                                    , pi_to_pi(iyaw + q0[3]), directions;
            }
    ...
    Path RSPaths::calc_shortest_path(Eigen::Vector4d s, Eigen::Vector4d g, double maxc, double step_size){
        ...
        Path tmp;
        return (paths.size() >0 ) ? paths[best_path_index] : tmp;
    }

    double RSPaths::calc_shortest_path_length(Eigen::Vector4d s, Eigen::Vector4d g, double maxc, double step_size){
    ...
    void RSPaths::check_path(Eigen::Vector4d s, Eigen::Vector4d g, double max_curvature){
        std::vector<Path> paths;
