...
# define a function to calculate Euclidean distance between points
...
knot_vector = np.linspace(0, 20, data.shape[0])
b_spline_x = CubicSpline(knot_vector, data[:,0])
b_spline_y = CubicSpline(knot_vector, data[:,1])
...
# Calculate arc length for each segment and cumulative distance (s_list)
s_list = [0]
length = 0
dx = b_spline_x(time_vec, 1)
dy = b_spline_y(time_vec, 1)
theta_list = np.arctan2(dy, dx)[1:]
y_list = np.array(y_list)
x_list = np.array(x_list)

distance_vec = []
distance_vec.append(0.0)
for i in range(1, len(x_list)):
    dis = calculate_distance([x_list[i], y_list[i]], [x_list[i - 1], y_list[i - 1]])
    length += dis
    s_list.append(s_list[-1] + dis)
...
# Define optimization variables including distance d
x = ca.MX.sym('x', n - 1)
y = ca.MX.sym('y', n - 1)
...
d = ca.MX.sym('d', n - 2)         # Distance variable between waypoints

# Objective function
objective = 0

# Curvature and curvature rate terms
for i in range(n - 3):
...
# Distance penalty
# for i in range(n - 3):
#     objective += distance_weight * (d[i+1] - d[i])**2  # Penalize large distances

for i in range(n - 2):
    objective += distance_weight * (d[i])**2  # Penalize large distances
...
# Distance constraints: d[i] = sqrt((x[i+1] - x[i])^2 + (y[i+1] - y[i])^2)
# for i in range(n - 2):
#     g.append(d[i] - ca.sqrt((x[i+1] - x[i])**2 + (y[i+1] - y[i])**2))
#     lb_g.append(0)
#     ub_g.append(0)
...
for i in range(n - 1):
    g.append(v[i])
    lb_g.append(0.0)  # Minimum velocity
...
# Define NLP problem
nlp = {'x': ca.vertcat(x, y, theta, v, delta, d), 'f': objective, 'g': ca.vertcat(*g)}
...
# Initial guess
x0_guess = np.hstack((x_list[1:], y_list[1:], theta_list[1:], 
                      np.ones(n - 1) * v_max, np.zeros(n - 1), np.ones(n - 2)))
...
# x, y bounds
for i in range(n - 1):
    lbx.extend([-ca.inf, -ca.inf])  # No bounds on x, y
...
for i in range(n - 1):
    lbx.append(-max_steering_angle)
    ubx.append(max_steering_angle)

# Distance bounds
for i in range(n - 2):
    lbx.append(0.1)  # Non-negative distances
    ubx.append(ca.inf)

# Solve the problem
solution = solver(x0=x0_guess, lbx=lbx, ubx=ubx, lbg=lb_g, ubg=ub_g)

# Extract results
x_opt = solution['x'][:n - 1]
y_opt = solution['x'][n - 1:2 * (n - 1)]
theta_opt = solution['x'][2 * (n - 1):3 * (n - 1)]
v_opt = solution['x'][3 * (n - 1):4 * (n - 1)]
delta_opt = solution['x'][4 * (n - 1):5 * (n - 1)]
d_opt = solution['x'][5 * (n - 1):]
...
# # Bounds for x and y positions (no bounds in this example, set to -10000, 10000)
# for i in range(n - 1):
#     lbx.extend([-ca.inf, -ca.inf])  # x and y lower bounds
#     ubx.extend([ca.inf, ca.inf])   # x and y upper bounds

# # Bounds for theta (no specific bounds in this example, set to -10000, 10000)
# for i in range(n - 1):
#     lbx.append(-ca.pi)  # theta lower bound
#     ubx.append(ca.pi)   # theta upper bound

# # Bounds for v (velocity) - [0, v_max] (replaced with -10000, 10000 here)
# for i in range(n - 1):
#     lbx.append(-v_max)  # velocity lower bound
#     ubx.append(v_max)   # velocity upper bound

# # Bounds for delta (steering angle) (no bounds, set to -10000, 10000)
# for i in range(n - 1):
#     lbx.append(-ca.pi/3)  # steering angle lower bound
#     ubx.append(-ca.pi/3)   # steering angle upper bound
